...
def cache_checkout_data(request):
    try:
        ...
        stripe.PaymentIntent.modify(pid, metadata={
            'retreat': json.dumps(request.session.get('retreat', {})),
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            order.original_retreat = json.dumps(retreat)
            order.save()
        ...
        retreat = request.session.get('retreat', {})
        if not retreat:
            messages.error(request, "You haven't added anything to your \
                retreat yet.")
            return redirect(reverse('all_stay_services'))

    current_retreat = retreat_contents(request)
    total = current_retreat['total']
